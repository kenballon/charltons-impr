...
function getNewsPostItems($atts = [])
{
    $atts = shortcode_atts([
        'categories' => 'news', // comma/space/| separated slugs
        'posts_per_page' => 15,     // preferred attribute
        'limit' => null,   // back-compat
        'orderby' => 'date',
        'order' => 'DESC',
        'match' => 'any',  // any|all
        'cache' => 300,    // seconds; 0 disables
    ], $atts, 'get_news_post_items');

    // Normalize categories
    $raw_slugs = preg_split('/[\s,\|]+/', (string) $atts['categories'], -1, PREG_SPLIT_NO_EMPTY);
    $slugs = array_values(array_unique(array_filter(array_map('sanitize_title', array_map('strtolower', $raw_slugs)))));
    if (empty($slugs)) {
        $slugs = ['news'];
    }

    // posts_per_page with cap (support old "limit")
    $ppp = isset($atts['posts_per_page']) && $atts['posts_per_page'] !== '' ? (int) $atts['posts_per_page'] : (int) $atts['limit'];
    $ppp = max(1, min(100, $ppp ?: 15));

    // orderby/order
    $allowed_orderby = ['date', 'modified', 'title', 'menu_order', 'rand'];
    $orderby = in_array(strtolower((string) $atts['orderby']), $allowed_orderby, true) ? strtolower((string) $atts['orderby']) : 'date';
    $order = strtoupper((string) $atts['order']) === 'ASC' ? 'ASC' : 'DESC';
    $operator = (strtolower((string) $atts['match']) === 'all') ? 'AND' : 'IN';

    // Transient cache
    $cache_ttl = max(0, (int) $atts['cache']);
    $cache_key = 'cp_sc_' . md5(wp_json_encode([
        'slugs' => $slugs,
        'ppp' => $ppp,
        'orderby' => $orderby,
        'order' => $order,
    ...
    $q = new WP_Query([
        ...
        'tax_query' => [
            [
                ...
                'field' => 'slug',
                'terms' => $slugs,
                'operator' => $operator, // AND=all terms, IN=any term
            ]
        ],
        'meta_query' => [
            [
                'key' => '_thumbnail_id',
                'compare' => 'EXISTS', // only posts with featured image
            ]
        ],
    ]);
    ...
    while ($q->have_posts()) {
        ...
        // Optional: allow custom image meta to override featured image
        $custom_img = get_post_meta($post_id, 'fiuw_image_url', true);
        $img_url = $custom_img ? $custom_img : get_the_post_thumbnail_url($post_id, 'medium_large');
